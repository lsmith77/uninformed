...
CREATE TABLE documents (document_id BIGINT AUTO_INCREMENT, name VARCHAR(255) NOT NULL, publication_date DATE, adoption_date DATE NOT NULL, code VARCHAR(255), min_ratification_count INT, preamble TEXT, parent_id BIGINT, organisation_id BIGINT, documenttype_id BIGINT, documenturl VARCHAR(255), created_at DATETIME, updated_at DATETIME, INDEX parent_id_idx (parent_id), INDEX organisation_id_idx (organisation_id), INDEX documenttype_id_idx (documenttype_id), PRIMARY KEY(document_id)) DEFAULT CHARACTER SET utf8 ENGINE = InnoDB;
